  ...
  initialized_(false)
{
  ...
  pose_subscriber_.reset(new PoseSubscriber(
    nh_, "/orb_slam2_rgbd/pose",10));

  
  // ros::Subscriber ppose_sub = nh_.subscribe("/orb_slam2_rgbd/pose",pposeCallback,10);
  ...
  else
  {
    ...
    pose_subscriber_->registerCallback(&ImuFilterRos::pposeCallback, this);
  }
...
void ImuFilterRos::pposeCallback(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    // ppose.header.seq = msg->header.seq;
    // ppose.header.stamp = msg->header.stamp;
    // ppose.header.frame_id = "camera_link";
    ROS_INFO("ppose Callback ongoing");
    // geometry_msgs::PoseStamped ppose;
    // ppose.pose.position.x = msg->pose.position.x;
    // ppose.pose.position.y = msg->pose.position.y;
    // ppose.pose.position.z = msg->pose.position.z;

    filter_.setPose(msg->pose.position.x,msg->pose.position.y,msg->pose.position.z);

    // cout<<ppose.pose.position.x<<endl;
}
...
void ImuFilterRos::publishTransform(const ImuMsg::ConstPtr& imu_msg_raw)
{
  ...
  double ppose_x,ppose_y,ppose_z;
  filter_.getPose(ppose_x,ppose_y,ppose_z);
  ...
  if (reverse_tf_)
  {
    ...
    transform.transform.translation.x = ppose_x;
    transform.transform.translation.y = ppose_y;
    transform.transform.translation.z = ppose_z;
    // ROS_INFO("pose_position published");
    ...
    transform.transform.translation.y = ppose_y;
    transform.transform.translation.z = ppose_z;
    // ROS_INFO("pose_position publishing");
